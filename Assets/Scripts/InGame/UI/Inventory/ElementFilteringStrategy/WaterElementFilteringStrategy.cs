...
namespace InGame.UI
{
    public class WaterElementFilteringStrategy : AbstractInventoryFilteringStrategy
    {
        ...
        {
            var filtered = data.Where(x => x.Value.getItem().findAbility("Water")).ToDictionary(x => x.Key, x => x.Value);
            return filtered;
        }
