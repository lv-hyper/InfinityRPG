...
namespace InGame.UI
{
    public class ShowingBuyableStrategy : AbstractInventoryFilteringStrategy
    {
        ...
        {
            var filtered = data.Where(x => x.Value.getItem().price > 0).ToDictionary(x => x.Key, x => x.Value);
            return filtered;
        }
