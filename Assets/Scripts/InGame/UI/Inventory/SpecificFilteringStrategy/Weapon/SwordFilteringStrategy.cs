...
namespace InGame.UI
{
    public class SwordFilteringStrategy : AbstractInventoryFilteringStrategy
    {
        ...
        {
            var filtered = data.Where(x => x.Value.getItem().GetAdditionalInfo().Contains("STR")).ToDictionary(x => x.Key, x => x.Value);
            return filtered;
        }
